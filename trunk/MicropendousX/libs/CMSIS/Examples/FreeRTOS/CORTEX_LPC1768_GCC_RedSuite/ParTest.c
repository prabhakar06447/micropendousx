...
*/
// NXP: Change to P2.3 --> 6
#define LED_2 ( 1UL << 3UL )
#define LED_3 ( 1UL << 4UL )
#define LED_4 ( 1UL << 5UL )
#define LED_5 ( 1UL << 6UL )
...
void vParTestInitialise( void )
{
	...
	// NXP: Change to P2
	LPC_GPIO2->FIODIR  = partstFIO1_BITS;

	/* Start will all LEDs off. */
	// GPIO1->FIOCLR = partstFIO1_BITS;
	// NXP: Change to P2
	LPC_GPIO2->FIOCLR = partstFIO1_BITS;
}
/*-----------------------------------------------------------*/
...
void vParTestSetLED( unsigned portBASE_TYPE uxLED, signed portBASE_TYPE xValue )
{
	if( uxLED < partstNUM_LEDS )
	{
		...
		if( xValue )
		{
			...
			// NXP: Change to P2
			LPC_GPIO2->FIOCLR = ulLEDs[ uxLED ];
		}
		else
		{
			// GPIO1->FIOSET = ulLEDs[ uxLED ];
			// NXP: Change to P2
			LPC_GPIO2->FIOSET = ulLEDs[ uxLED ];
		}
	}
}
/*-----------------------------------------------------------*/
...
void vParTestToggleLED( unsigned portBASE_TYPE uxLED )
{
	if( uxLED < partstNUM_LEDS )
	{
		...
		*/
		// NXP: Change to P2
		if( LPC_GPIO2->FIOPIN & ulLEDs[ uxLED ] )
		{
			LPC_GPIO2->FIOCLR = ulLEDs[ uxLED ];
		}
		else
...
unsigned portBASE_TYPE uxParTextGetLED( unsigned portBASE_TYPE uxLED )
{
	...
	*/
	...
	if( uxLED < partstNUM_LEDS )
	{
		return ( LPC_GPIO2->FIOPIN & ulLEDs[ uxLED ] );
	}
	else
	{
		return 0;
