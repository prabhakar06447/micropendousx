...
static long prvWritePHY( long lPhyReg, long lValue )
{
	...
	if (dev_175x == __TRUE) {
		/* LPC1758 does not have functional MIIM - use software per AN10859 */
		mdio_write(lPhyReg, lValue);
		return pdPASS; /* TODO - actually check if write was successful */

	} else {
		/* LPC176x - normal PHY operation */
		LPC_EMAC->MADR = PHY_DEF_ADR | lPhyReg;
...
static unsigned short prvReadPHY( unsigned char ucPhyReg, long *plStatus )
{
	...
	if (dev_175x == __TRUE) {
		/* LPC1758 does not have functional MIIM - use software per AN10859 */
		return mdio_read(ucPhyReg);

	} else {
		/* LPC176x - normal PHY operation */
		LPC_EMAC->MADR = PHY_DEF_ADR | ucPhyReg;
